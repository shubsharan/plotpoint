...
import type { StoryNode, StoryEdge, StoryManifest } from "@plotpoint/db";
import type { ComponentContext, ComponentTypeName, VersionConstraint } from "@plotpoint/schemas";
import {
  componentRegistry,
...
export function NodeRenderer({
  ...
  const resolvedComponent = useMemo(() => {
    ...
    const requiredComponents = manifest?.requiredComponents as
      | Record<ComponentTypeName, VersionConstraint>
      | undefined;
    if (requiredComponents?.[nodeType]) {
      const constraint = requiredComponents[nodeType];
      return {
