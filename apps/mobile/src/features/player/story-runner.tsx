...
import type { Story, StoryNode, StoryEdge, StoryManifest } from "@plotpoint/db";
import type { GameState, InventoryItem, ComponentContext, EdgeCondition } from "@plotpoint/schemas";

// Local session state with proper typing
interface LocalSessionState {
  ...
  visitedNodes: string[];
  choiceHistory: Array<{ nodeId: string; edgeId: string; timestamp: string }>;
}
...
interface StoryRunnerContextValue {
  ...
  // Session state
  session: LocalSessionState | null;
  gameState: GameState;
  inventory: InventoryItem[];
  visitedNodes: string[];
...
function evaluateCondition(
  ...
  switch (condition.operator) {
    ...
    case "and":
      return ((condition.conditions as EdgeCondition[]) ?? []).every((c) =>
        evaluateCondition(c, gameState, inventory),
      );

    case "or":
      return ((condition.conditions as EdgeCondition[]) ?? []).some((c) =>
        evaluateCondition(c, gameState, inventory),
      );
...
function useStoryEdges(storyId: string) {
  return useQuery({
    ...
    queryFn: async () => {
      ...
      if (nodeIds.length === 0) return [] as unknown as StoryEdge[];
...
export function StoryRunner({ storyId, userId = "anonymous" }: StoryRunnerProps) {
  ...
  const [localSession, setLocalSession] = React.useState<LocalSessionState>({
    gameState: {},
    inventory: [],
    visitedNodes: [],
    choiceHistory: [],
  ...
  React.useEffect(() => {
    if (sessionQuery.data) {
      // Convert DB session to local state format
      const dbSession = sessionQuery.data;
      setLocalSession({
        id: dbSession.id,
        currentNodeId: dbSession.currentNodeId,
        gameState: (dbSession.gameState as GameState) ?? {},
        inventory: (dbSession.inventory as InventoryItem[]) ?? [],
        visitedNodes: (dbSession.visitedNodes as string[]) ?? [],
        choiceHistory:
          (dbSession.choiceHistory as Array<{
            nodeId: string;
            edgeId: string;
            timestamp: string;
          }>) ?? [],
  ...
  const updateSessionMutation = useMutation({
    mutationFn: async (updates: Partial<LocalSessionState>) => {
      const { error } = await supabase.from("story_sessions").upsert({
        user_id: userId,
        story_id: storyId,
        current_node_id: updates.currentNodeId ?? localSession.currentNodeId,
        game_state: updates.gameState ?? localSession.gameState,
        inventory: updates.inventory ?? localSession.inventory,
        visited_nodes: updates.visitedNodes ?? localSession.visitedNodes,
        choice_history: updates.choiceHistory ?? localSession.choiceHistory,
  ...
  const navigateToNode = useCallback(
    (nodeId: string) => {
      ...
      const updates: Partial<LocalSessionState> = {
  ...
  const updateGameState = useCallback(
    (stateUpdates: Partial<GameState>) => {
      const newState: GameState = { ...localSession.gameState, ...stateUpdates };
      const updates: Partial<LocalSessionState> = { gameState: newState };
  ...
  const updateInventory = useCallback(
    (item: InventoryItem, action: "add" | "remove" | "update") => {
      const newInventory = [...localSession.inventory];
      ...
      const updates: Partial<LocalSessionState> = { inventory: newInventory };
      setLocalSession((prev) => ({ ...prev, ...updates }));
      updateSessionMutation.mutate(updates);
    },
  ...
  const restartStory = useCallback(() => {
    ...
    const updates: LocalSessionState = {
  ...
  const componentContext = useMemo<ComponentContext>(
    () => ({
      ...
      shellType: storyQuery.data?.shellType ?? "ebook",
