...
CREATE TYPE "run_role_slot_status" AS ENUM('pending', 'assigned', 'active', 'completed');--> statement-breakpoint
...
CREATE TABLE "run_role_slots" (
	"run_id" text,
	"role_id" text,
	"status" "run_role_slot_status" DEFAULT 'pending'::"run_role_slot_status" NOT NULL,
	"created_at" timestamp with time zone DEFAULT now() NOT NULL,
...
ALTER TABLE "story_runs" ADD CONSTRAINT "story_runs_o23konntepec_fkey" FOREIGN KEY ("story_package_version_id") REFERENCES "story_published_package_versions"("id") ON DELETE SET NULL;
