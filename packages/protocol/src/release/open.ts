...
import type { CanonicalJsonObject, InvalidRelease, OpenedRelease, ReleaseEntry } from "./types.js";
...
function immutableEntry(entry: ReleaseEntry): ReleaseEntry {
  const captured = new Uint8Array(entry.bytes);
...
export async function openRelease(
...
): Promise<OpenedRelease | InvalidRelease> {
  ...
  for (const expected of manifest.inventory) {
    ...
    entries.push(immutableEntry({ ...expected, bytes: actual.bytes }));
  }
  return Object.freeze({
    kind: "opened",
