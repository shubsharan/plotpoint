...
- [ ] T040 [US1] Replace public game-specific session routes/operator calls with `/shared-sessions` release, creation, invitation, join, revoke, command, and pull operations in `apps/api/src/server.ts` and `apps/api/src/operator-client.ts`
