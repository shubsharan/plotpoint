...
It corrects the private pre-release payloads in place. The Host API retains its local core and
shared-play extension; this feature introduces no new message-name generation, payload discriminator,
...
owns executable model selection and translates between Host API and the unversioned runtime. Release
...
The existing `runtime.ready` request receives `runtime.bootstrap`. Both use Host Bridge Envelope with
the centrally registered `CONTRACT_VERSIONS.hostBridge` value and their existing direction. The
bootstrap is consumed by the generated runtime adapter before the application mounts.
