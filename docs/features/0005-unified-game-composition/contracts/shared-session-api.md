...
envelopes remain. HTTPS is required outside loopback tests. JSON stays bounded at 256 KiB and
...
- `POST /shared-sessions`: operator supplies `version`, `creationId`, `releaseId`, and `teamLabel`.
...
- `POST /shared-sessions/{sessionId}/invitations`: operator supplies `version`, `invitationId`, and
  `expiresAt`; one raw one-use invitation is returned once and only its keyed digest persists.
- `POST /shared-sessions/{sessionId}/participants`: unauthenticated join supplies `version`,
  `joinRequestId`, `expectedReleaseId`, invitation, and native-generated participant credential.
- `POST /shared-sessions/{sessionId}/participants/{participantId}/revoke`: operator supplies
...
Transport/policy failures use a closed
`{ version: typeof CONTRACT_VERSIONS.sharedApi, code, requestId }` object. Unknown, expired,
