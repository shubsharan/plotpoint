...
identity, schema ID, and payload. The adapter validates the payload through the exact
...
identity, manifest digest, and payload before persistence or component exposure.

## Target Discovery

The first closed registry entry is `plotpoint.location.target-discovery`. It requires one
...
Game Composition, inventory relationships, the exact adapter identity, all digest-bound
...
Stable failures distinguish unknown mechanic, invalid binding/configuration,
...
A new mechanic requires another explicit registry identity. If incompatible mechanic evolution is ever
needed, a centralized compatibility mechanism will govern it without renaming every adapter or schema.
