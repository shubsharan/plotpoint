...
inventoried content path. Release Format and Host API compatibility metadata stay unchanged. The public `inspect`
...
application catalog can evolve inside Release Format's existing application-content
...
Game Composition. The binding names the platform mechanic ID/version, data-only server aggregate
...
**Decision**: Preserve Shared Play and Sync wire shapes. Use the existing
