...
- [x] **SC-008 Total preflight invalidity**: Cyclic aggregate/command values, malformed observation scripts, and invalid policies return typed preflight failures without records or exceptions; every post-preflight variant carries a record.
- [x] **SC-009 Aggregate-kind type safety**: Type fixtures reject command, aggregate, definition, progression, fixture, result, and replay kind mismatches.
...
- [x] `pnpm lint`
- [x] `pnpm check-types`
- [x] `pnpm test` - 18 files and 78 tests passed.
- [x] `pnpm build`
- [x] `pnpm --filter @plotpoint/runtime bench` - representative 20-node parallel progression baseline completed at approximately 11,632 operations/second without a pass/fail threshold.
- [x] `pnpm speckit:test`
