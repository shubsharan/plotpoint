...
in place with plain names. Git history records the discarded shapes. ADR 0006 governs centralized
contract evolution and removes independent per-schema generations.
...
2. A state-specific `ResolvedAggregateModel` owns model identity, aggregate kind,
...
3. The existing `Aggregate` gains model and schema identity and retains `stateVersion` as
...
5. `plotpoint.project.json` remains strict Project Configuration and is the sole authored
   ...
   upgrade path for the discarded pre-release shape.
...
9. Host Bridge Envelope and the existing message names `runtime.ready`, `runtime.bootstrap`,
   ...
   is the shared-play extension; no additional Host API minor is introduced. Runtime Bootstrap and
   Local Transition are corrected in place. The generated runtime adapter consumes bootstrap and
...
11. Host API 1.1 Shared Play and Sync wire semantics remain. Shared outbox rows use
...
- Release Format, Game Composition, Host API, Sync, Trusted Mechanic, Shared Recovery, and Game Play
  Report use plain names; centralized format and boundary metadata owns compatibility.
- Runtime, progression, interface, and schema names evolve in place because they are repository-owned
  pre-release APIs. Future incompatible evolution follows ADR 0006.
