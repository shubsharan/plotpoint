...
2. The host generates a bootstrap document from verified release entrypoints. It locks navigation,
   ...
   serializable bridge with centrally registered compatibility metadata.
...
- A future stronger isolation boundary may change the host API while release-format remains
  unchanged.
