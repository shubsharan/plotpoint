...
2. The compiler snapshots every selected byte, statically analyzes separate deterministic-logic and
   browser-presentation import graphs, rejects forbidden or unresolved authority, and bundles only
   the captured graph. It may evaluate selected Gate 1 definition modules only in a bounded local
   build subprocess after import validation, and it never invokes handlers or progression predicates.
   This is process isolation, not a hostile-code sandbox; hosted compilation requires a separately
   accepted isolated-executor design.
3. Release-format version 1 is one `.pprelease` file using a strict ZIP-compatible profile: stored
...
4. `manifest.json` is RFC 8785 canonical JSON. It declares the release-format version, bounded host
...
5. A release identity is `sha256:<64 lowercase hexadecimal characters>` over every finalized
   ...
   a sub-digest.
6. Non-executing verification validates the strict container, canonical manifest, exact inventory,
...
7. Release format, host API, and aggregate schema compatibility remain independent. Release-format
...
8. `@plotpoint/protocol` owns portable manifest, container, inspection, verification, and
   compatibility semantics. `@plotpoint/compiler` owns project validation, graph analysis,
   composition, bundling, and same-filesystem atomic publication through a temporary file. A player
   consumes protocol contracts and never depends on the compiler.
...
- The portable protocol surface becomes a compatibility obligation for future players and tools.
